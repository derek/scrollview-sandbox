...
Y.ScrollView = Y.extend(ScrollView, Y.Widget, {
    ...
    _onGestureMove: function(e) {
        ...
        if (gesture.isVertical && axisY) {
            delta = -(e.clientY - gesture.startY);
            sv.set(SCROLL_Y, delta);
        }
        else if (axisX) {
            sv.set(SCROLL_X, -(e.clientX - gesture.startX));
        }
    },
    ...
    _onGestureMoveEnd: function(e) {
        ...
        var sv = this, // kweight
            ...
            minX = sv._minScrollX,
            maxX = sv._maxScrollX,
            startPoint =  isVertical ? gesture.startClientY : gesture.startClientX,
            endPoint = isVertical ? gesture.endClientY : gesture.endClientX,
        ...
        // Detach the event listeners assigned in _onGestureMoveStart
         ...
         * @public
         */
        sv.lastScrolledAmt = distance + (sv.lastScrolledAmt || 0);

        // Check for halfway
        if(( isVertical && absDistance > (offsetHeight/2)) || 
           (!isVertical && absDistance > (offsetWidth/2))) {
        ...
        // Check for min/max
        if (isVertical) {
            y = _constrain(currentY, minY, maxY);
        }

        if (!isVertical) {
            x = _constrain(currentX, minX, maxX);
        }
    ...
    _flickFrame: function() {
        var sv = this,
            ...
            velocity = sv._currentVelocity,
            newX = currentX - (velocity * step),
            newY = currentY - (velocity * step);

        velocity = sv._currentVelocity = (velocity * deceleration);
